    ...
    def select_checkboxes():
        ...
        if not st.session_state.submitted:
            ...
            with st.form("my_form"):
                ...
                selected = {}
                for opt in options:
                    selected[opt] = st.checkbox(opt, key=opt)
            ...
            if submitted:
                ...
                checked = [opt for opt, is_checked in selected.items() if is_checked]

                if len(checked) == 0:
                    st.warning("⚠️ You must select one option.")
                    st.rerun()
                elif len(checked) > 1:
                    st.error("❌ Please select only one option.")
                    st.rerun()
                else:
                    st.success(f"✅ You selected: {checked[0]}")
                    st.session_state.selected.append(checked[0])
                    st.rerun()
