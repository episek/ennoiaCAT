...
st.sidebar.image('cisco_logo.png')
st.sidebar.image('ennoia_white_black_hi-def.png')
st.title("Cisco Systems and")
...
try:
    ...
    ip = "192.168.1.36"  # IP address of the system
...
if lang:
    ...
    selected_options = CSHelper.select_checkboxes()
    ...
    if "SLM" in selected_options:
        ...
        def load_model_and_tokenizer():
            return CSHelper.load_lora_model()
        ...
        translated = GoogleTranslator(source='auto', target=lang).translate(text)
        st.write(translated)
        client, ai_model = CSHelper.load_OpenAI_model()
        map_api = MapAPI() 
    ...
    if serial_ports:
        selected_serial = st.selectbox("Selected Serial Port", serial_ports, index=0)
    else:
    ...
    if st.button("Connect Serial"):
        try:
            st.session_state.conn = NCS540Serial(
                port=selected_serial, username=username, password=password
            )
            st.session_state.connected_port = selected_serial
            st.session_state.conn.start_keep_alive(interval=60)  # 🔹 keep alive
            st.success(
                f"✅ NCS540 Connected and logged in via **{selected_serial}** as {username} "
                "(login active, keep-alive enabled)"
            )
        except Exception as e:
            st.error(f"❌ Failed to connect/login: {e}")

    # 🔹 Halt UI until connected
    if not st.session_state.conn:
        st.warning("⚠️ Not connected. Please press **Connect Serial** to continue.")
        st.stop()

    # ✅ If connected, keep green banner + allow CLI
    st.success(f"✅ NCS540 Connected on **{st.session_state.connected_port}** as {username} (session active)")
    ...
    if st.session_state.conn:
        ...
        if st.button("Fetch Device Info"):
            ...
            with st.expander("show inventory (Router HW)"):
                st.text(inv)
            with st.expander("show version (Router SW)"):
                st.text(ver)
            with st.expander("show platform (Router Operation)"):
                st.text(plat)

        st.subheader("Router Connectivity Info")
        user_cmd = st.text_input("Command", "show l2vpn bridge-domain ; show monitor-session 1 status")
    ...
    eths = list_eth_interfaces()
    ip_list = list(eths.values())
    iface_by_ip = {ip: iface for iface, ip in eths.items()}

    # === Dropdowns
    default_ip_out = "192.168.1.36"
    options = ["-- Select an IP --"] + ip_list
    default_index = options.index(default_ip_out) if default_ip_out in options else 0

    selected_ip_out = st.selectbox("Selected Output IP", options, index=default_index, key="output_ip")
    #selected_ip_out = st.selectbox("Output IP", ["-- Select an IP --"] + ip_list, key="output_ip")

    default_ip_in = "192.168.2.37"
    options = ["-- Select an IP --"] + ip_list
    default_index = options.index(default_ip_in) if default_ip_in in options else 0

    selected_ip_in = st.selectbox("Selected Capture IP", options, index=default_index, key="capture_ip")
    #selected_ip_in  = st.selectbox("Capture IP", ["-- Select an IP --"] + ip_list, key="capture_ip")

    # === Guard: require both selections
    if selected_ip_out == "-- Select an IP --" or selected_ip_in == "-- Select an IP --":
        st.warning("⚠️ Please select both Output and Capture IPs to continue.")
        st.stop()

    # === Map IPs to interface names
    iface_out = iface_by_ip[selected_ip_out]
    iface_in  = iface_by_ip[selected_ip_in]

    # === Save globally into session_state
    st.session_state.iface_out = iface_out
    st.session_state.iface_in = iface_in
    st.session_state.selected_ip_out = selected_ip_out
    st.session_state.selected_ip_in = selected_ip_in

    st.success(f"✅ Output Link: {selected_ip_out} ({iface_out})")
    st.success(f"✅ Capture Link: {selected_ip_in} ({iface_in})")

    # === Call helper only after both are set
    use_selected_interfaces()       
    ...
    # --- Get and cache the CS port ---
    if "Cisco_port" not in st.session_state:
        st.session_state.CS_port = helper.getport()
    ...
    if prompt:
        ...
        if "SLM" in selected_options:
            ...
            if "capture" in prompt:
                def_dict["capture"] = True
            else:
                def_dict["capture"] = False
        ...
        if isinstance(api_dict, dict):
            ...
            print(f"opt = {opt}")
            helper.configure_CS(opt,lang,d)
            if opt.port:
                if (d == 0):
                    text = ("PCAP Porting to NCS540 Successfully Completed!")
                    translated = GoogleTranslator(source='auto', target=lang).translate(text)
                    st.write(translated)
        else:
